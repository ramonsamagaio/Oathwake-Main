...
var _water: PixelWaterContainer2D
...
func _find_water() -> void:
    var nodes := get_tree().get_nodes_in_group("pixel_water_container")
    if nodes.is_empty():
    ...
    var best: PixelWaterContainer2D = null
    var best_distance := INF
    for node in nodes:
        var candidate := node as PixelWaterContainer2D
        if candidate == null:
            continue
        var dx := 0.0
        if global_position.x < candidate.basin_left:
            dx = candidate.basin_left - global_position.x
        elif global_position.x > candidate.basin_right:
            dx = global_position.x - candidate.basin_right
        if dx < best_distance:
            best_distance = dx
            best = candidate
    _water = best
...
func _update_prediction() -> void:
    ...
    predicted_submerged_fraction = (
        mass / maxf(displaced_mass_capacity, 0.0001)
    )
...
func _physics_process(delta: float) -> void:
    if (
        _water == null
        or global_position.x < _water.basin_left - object_size_px.x
        or global_position.x > _water.basin_right + object_size_px.x
    ):
        _find_water()
    ...
    submerged_fraction = (
        float(submerged_points.size())
        / float(_sample_points.size())
    )
    _update_prediction()

    var displaced_volume := object_volume_m3 * submerged_fraction
    _water.report_displacement(get_instance_id(), displaced_volume)
    ...
    # Archimedes: F_b = rho * g * displaced volume.
    var volume_per_sample := (
        object_volume_m3 / float(_sample_points.size())
    )
    ...
    # Quadratic drag: Fd = 1/2 rho Cd A v².
    ...
    if speed_m_s > 0.01:
        ...
        var projected_area_m2 := maxf(
            0.0001,
            projected_length_m * physical_depth_m
        )
        var drag_newtons := (
            0.5
            * _water.water_density_kg_m3
        ...
        # Do not let an explicit force reverse the entire body in one frame.
        var max_drag := (
            mass
...
func float_state_text() -> String:
    if predicted_submerged_fraction < 0.98:
        return (
            "FLOATS  •  equilibrium ≈ %d%% submerged"
            % int(
                clampf(
                    predicted_submerged_fraction,
                    0.0,
