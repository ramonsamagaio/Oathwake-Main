...
@export_range(3, 9, 1) var sample_columns: int = 6
@export_range(3, 9, 1) var sample_rows: int = 5
...
var _water: PixelWaterSimulator2D
...
func _find_water() -> void:
    var nodes := get_tree().get_nodes_in_group("water_simulator")
    if not nodes.is_empty():
        _water = nodes[0] as PixelWaterSimulator2D
...
func _update_prediction() -> void:
    if _water == null:
        return
    var displaced_mass_capacity := _water.water_density_kg_m3 * object_volume_m3 * buoyancy_multiplier
...
func _physics_process(delta: float) -> void:
    ...
    if _dragging:
        ...
        _drag_velocity = _drag_velocity.lerp((mouse - _last_mouse_pos) / maxf(delta, 0.0001), 0.45)
    ...
    # Archimedes: F_b = rho_fluid * g * displaced_volume.
    # Godot's gravity is in px/s², so using kg and px/s² keeps the force coherent
    # with RigidBody2D's mass while the volume itself remains expressed in m³.
    var volume_per_sample := object_volume_m3 / float(_sample_points.size())
    var force_per_sample := _water.water_density_kg_m3 * _water.gravity_px_s2 * volume_per_sample * buoyancy_multiplier
    last_buoyant_force = force_per_sample * float(submerged_points.size())
    for world_point in submerged_points:
        apply_force(Vector2(0.0, -force_per_sample), world_point - global_position)

    # Quadratic hydrodynamic drag: F_d = 1/2 rho C_d A v².
    # Velocity is converted from px/s to m/s, then force back to kg*px/s².
    ...
    if speed_m_s > 0.01:
        ...
        var projected_length_m := absf(direction.y) * width_m + absf(direction.x) * height_m
        var projected_area_m2 := maxf(0.0001, projected_length_m * physical_depth_m)
        var drag_newtons := 0.5 * _water.water_density_kg_m3 * drag_coefficient * projected_area_m2 * speed_m_s * speed_m_s
        var drag_force_px := -linear_velocity.normalized() * drag_newtons * pixels_per_meter * submerged_fraction
        var max_drag := mass * maxf(linear_velocity.length(), 1.0) / maxf(delta, 0.001) * 0.72
    ...
    if just_entered and linear_velocity.y > 35.0:
        _water.register_object_impact(global_position.x, mass, linear_velocity.y, object_volume_m3 * submerged_fraction, object_size_px.x)

    var sink_ratio := maxf(0.0, predicted_submerged_fraction - 1.0)
    _water.register_underwater_motion(global_position, linear_velocity, maxf(object_size_px.x, object_size_px.y), sink_ratio, delta)
...
func _end_drag() -> void:
    ...
    freeze = false
    sleeping = false
    linear_velocity = _drag_velocity.limit_length(1450.0)
...
func float_state_text() -> String:
    if predicted_submerged_fraction < 0.98:
        return "FLOATS  •  equilibrium ≈ %d%% submerged" % int(clampf(predicted_submerged_fraction, 0.0, 1.0) * 100.0)
...
func _draw() -> void:
    ...
    if shape_kind == "circle":
        ...
        draw_circle(Vector2(-half.x * 0.25, -half.y * 0.25), maxf(2.0, minf(half.x, half.y) * 0.13), _material_color.lightened(0.26))
    else:
        draw_rect(rect, _material_color)
        draw_rect(Rect2(rect.position + Vector2(3.0, 3.0), Vector2(maxf(3.0, rect.size.x - 6.0), 3.0)), _material_color.lightened(0.20))
        draw_rect(Rect2(rect.position + Vector2(3.0, rect.size.y - 5.0), Vector2(maxf(3.0, rect.size.x - 6.0), 2.0)), _material_color.darkened(0.20))
