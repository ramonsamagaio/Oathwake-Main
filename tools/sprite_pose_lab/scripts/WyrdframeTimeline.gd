...
var min_column_width := 24.0
var max_column_width := 120.0
...
func set_cell_width(value: float) -> void:
	column_width = clampf(value, min_column_width, max_column_width)
	_refresh_size()
	queue_redraw()
...
func _draw_header() -> void:
	...
	draw_string(
		get_theme_default_font(), Vector2(10, 20), "CAMADAS / BONES",
		HORIZONTAL_ALIGNMENT_LEFT, label_width - 20, 12, Color(0.78, 0.82, 0.88)
	)
	for frame_number in range(project.frame_count()):
		...
		draw_string(
			get_theme_default_font(), Vector2(x, 20), str(frame_number + 1),
			HORIZONTAL_ALIGNMENT_CENTER, column_width, 12, Color(0.9, 0.92, 0.95)
		)
...
func _draw_rows() -> void:
	for row in range(project.bones.size()):
		...
		draw_string(
			get_theme_default_font(), Vector2(10 + depth * 14, y + 19), display_name,
			HORIZONTAL_ALIGNMENT_LEFT, label_width - 20 - depth * 14, 12, Color(0.86, 0.88, 0.92)
		)
		for frame_number in range(project.frame_count()):
	...
	if event is InputEventMouseButton:
		...
		if mouse_event.ctrl_pressed and mouse_event.pressed:
			if mouse_event.button_index == MOUSE_BUTTON_WHEEL_UP:
				set_cell_width(column_width + 4.0)
				accept_event()
				return
