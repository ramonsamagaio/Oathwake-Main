extends "res://tools/sprite_pose_lab/scripts/WyrdframeStudioInteraction.gd"
...
func _request_load_project() -> void:
	...
	_open_dialog(
		...
		["*.wyrd ; Wyrdframe Project", "*.json ; Projeto legado JSON"]
	)
...
func _on_file_selected(path: String) -> void:
	match file_action:
		...
		FileAction.SAVE_PROJECT:
			_write_project(_ensure_extension(path, pose_project.FILE_EXTENSION))
...
func _load_texture_path(path: String) -> void:
	...
	pose_canvas.clear_texture_cache()
	_mark_changed("PNG associado à ação %s / %s." % [pose_project.current_action, DIRECTION_LABELS.get(pose_project.current_direction, pose_project.current_direction)])


func _write_project(path: String) -> void:
	path = _ensure_extension(path, pose_project.FILE_EXTENSION)
	if _write_document(path, pose_project.to_document(), true):
...
func _load_project(path: String) -> void:
	...
	if not pose_project.load_document(document):
		...
		return
	current_project_path = path if path.get_extension().to_lower() == pose_project.FILE_EXTENSION else ""
	dirty = false
	selected_frame = 0
	selected_bone_id = "torso" if not pose_project.bone_by_id("torso").is_empty() else str(pose_project.bones[0].get("id", "root"))
	pose_canvas.clear_texture_cache()
	_rebuild_everything()
	_set_status("Projeto carregado. Arquivos legados são migrados para .wyrd ao salvar.")


func _write_autosave() -> void:
	_write_document(AUTOSAVE_PATH, pose_project.to_document(), false)


func _write_document(path: String, document: Dictionary, report: bool = true) -> bool:
...
func _read_document(path: String) -> Dictionary:
	...
	if not (parsed is Dictionary):
		_set_status("Arquivo de projeto inválido.", true)
		return {}
	return parsed
...
func _frame_filename(frame_number: int) -> String:
	return "%s_%s_%s_%02d.png" % [
		_safe_name(pose_project.asset_name, "asset"),
		_safe_name(pose_project.current_action, "action"),
...
func _record_history() -> void:
	if history_suspended:
		return
	var snapshot := pose_project.to_document()
...
func _save_layout() -> void:
	if workspace_vsplit == null:
		return
	var config := ConfigFile.new()
	config.set_value("splits", "workspace_vertical", workspace_vsplit.split_offset)
	config.set_value("splits", "main_horizontal", main_hsplit.split_offset)
	config.set_value("splits", "center_horizontal", center_hsplit.split_offset)
	config.set_value("timeline", "cell_width", timeline.column_width if timeline != null else 42.0)
...
func _load_layout() -> void:
	...
	if config.load(LAYOUT_PATH) != OK:
		return
	await get_tree().process_frame
	workspace_vsplit.split_offset = int(config.get_value("splits", "workspace_vertical", workspace_vsplit.split_offset))
	main_hsplit.split_offset = int(config.get_value("splits", "main_horizontal", main_hsplit.split_offset))
	center_hsplit.split_offset = int(config.get_value("splits", "center_horizontal", center_hsplit.split_offset))
	if timeline != null:
		timeline.set_cell_width(float(config.get_value("timeline", "cell_width", 42.0)))
	var saved_size: Variant = config.get_value("window", "size", Vector2i(1500, 900))
	if saved_size is Vector2i:
		get_window().size = Vector2i(maxi(1050, saved_size.x), maxi(680, saved_size.y))
	var saved_position: Variant = config.get_value("window", "position", get_window().position)
