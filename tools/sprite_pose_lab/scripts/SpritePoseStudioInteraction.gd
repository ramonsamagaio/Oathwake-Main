extends "res://tools/sprite_pose_lab/scripts/SpritePoseStudioPanels.gd"
...
func _refresh_ui() -> void:
	...
	_select_option_metadata(clip_option, pose_project.current_clip)
	var clip_data := pose_project.current_clip_data()
	clip_name_edit.text = str(clip_data.get("name", pose_project.current_clip))
...
func _refresh_inspector() -> void:
	...
	texture_path_label.text = "Sem PNG: placeholder ativo" if path.is_empty() else path
...
func _rebuild_clip_option() -> void:
	clip_option.clear()
	for clip_id in pose_project.clips.keys():
		var clip: Dictionary = pose_project.clips[clip_id]
		clip_option.add_item(str(clip.get("name", clip_id)))
		clip_option.set_item_metadata(clip_option.item_count - 1, str(clip_id))
...
func _apply_humanoid_preset() -> void:
	...
	selected_frame = 0
	_rebuild_everything()
	_mark_changed("Preset Humanoid Basic aplicado.")
...
func _remove_bone() -> void:
	if selected_bone_id == "root":
		...
		return
	_record_history()
	var old_id := selected_bone_id
	if pose_project.remove_bone(old_id):
...
func _on_clip_selected(_index: int) -> void:
	if updating_ui:
		return
	pose_project.current_clip = str(clip_option.get_selected_metadata())
	selected_frame = 0
	_refresh_ui()


func _on_clip_name_changed(value: String) -> void:
	if updating_ui:
		return
	pose_project.current_clip_data()["name"] = value
	_mark_changed()


func _add_clip() -> void:
	_record_history()
	pose_project.add_clip("animation", "Nova animação")
	selected_frame = 0
	_rebuild_everything()
	_mark_changed("Novo clip criado.")


func _apply_animation_preset(preset_id: String) -> void:
	_record_history()
	pose_project.apply_animation_preset(preset_id)
	selected_frame = 0
	_refresh_ui()
	_mark_changed("Preset de movimento aplicado como base editável.")
...
func _apply_gizmo_drag(canvas_position: Vector2, angle_snap: bool) -> void:
	...
	if drag_mode == pose_canvas.HANDLE_MOVE:
		...
		var bone := pose_project.bone_by_id(selected_bone_id)
		var constraints: Dictionary = bone.get("constraints", {})
		result_rotation = clampf(
			result_rotation,
			float(constraints.get("rotation_min", -720.0)),
			float(constraints.get("rotation_max", 720.0))
		)
