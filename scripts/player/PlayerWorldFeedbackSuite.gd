...
func _promote_screen_effects() -> void:
	...
	if screen_effects == null or scene_root == null or screen_effects.get_parent() == scene_root:
		return
	var existing := scene_root.get_node_or_null("ScreenEffects")
	if existing != null and existing != screen_effects:
		screen_effects.queue_free()
		return
	screen_effects.reparent(scene_root)
