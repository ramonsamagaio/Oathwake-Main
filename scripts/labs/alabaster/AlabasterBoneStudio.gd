...
func _ready() -> void:
	...
	_build_ui()
	_build_preview()
	_set_status("Bone Studio ready. Choose a Godot-imported FBX/GLB/GLTF/TS CN scene to begin.")
...
func _build_import_tab(tabs: TabContainer) -> void:
	...
	browse.pressed.connect(func(): file_dialog.popup_centered_ratio(0.75))
	...
	var note := Label.new()
	note.text = "Auto mapping handles Mixamo names. Every source bone below can be redirected manually if the imported skeleton uses different names or the automatic match deforms the pose."
...
func _build_manual_tab(tabs: TabContainer) -> void:
	...
	clear.pressed.connect(func(): manual_keys.clear(); _refresh_manual_key_list())
	preview_actions.add_child(clear)

	var hint := Label.new()
	hint.text = "The rendered sprites remain attached to the bones. Lower Sprite Opacity on the preview to inspect the skeleton while posing it. Tweening is quaternion interpolation in runtime, not pixel-frame morphing."
...
func _rebuild_mapping_table() -> void:
	...
	if rig != null and rig.has_method("get_bone_names"):
		targets = rig.call("get_bone_names") as Array[String]
...
func _save_import() -> void:
	...
	if data.is_empty():
		return
	var name := _sanitize_name(import_name_edit.text)
	if name.is_empty():
		_set_status("Choose a name for the imported animation.", true)
		return
	var meta := {"type":"retarget_import","source_path":source_path,"source_clip":_selected_clip(),"mapping":_get_mapping(),"settings":_get_import_settings()}
	if not Library.save_custom_animation(name, data, meta):
		_set_status("Could not save animation bank. Run the Bone Studio from the editable project, not an exported build.", true)
		return
	rig.install_runtime_animation(name, data)
	rig.set_animation(name)
	_set_status("Saved '%s' to %s." % [name, Library.CUSTOM_BANK_PATH])
...
func _build_import_animation() -> Dictionary:
	if source_path.is_empty() or source_clip_option.item_count <= 0:
		...
		return {}
	return Importer.import_scene_clip(source_path, _selected_clip(), import_fps.value, import_loop.button_pressed, 0.0, _get_mapping(), _get_import_settings())


func _selected_clip() -> String:
	return str(source_clip_option.get_item_metadata(source_clip_option.selected)) if source_clip_option.selected >= 0 else ""


func _get_mapping() -> Dictionary:
	var result := {}
	for source_bone in mapping_controls.keys():
		var option: OptionButton = mapping_controls[source_bone]
		result[source_bone] = str(option.get_item_metadata(option.selected)) if option.selected >= 0 else ""
	return result
...
func _populate_manual_bones() -> void:
	...
	if rig == null or not rig.has_method("get_bone_names"):
		return
	for bone_name in rig.call("get_bone_names"):
...
func _manual_select_key(index: int) -> void:
	...
	var xfm: Dictionary = (frame_data.get("nodeXfm", {}) as Dictionary).get(bone, {})
	var rot: Array = xfm.get("rot", [0.0,0.0,0.0])
	var trans: Array = xfm.get("trans", [0.0,0.0,0.0])
	manual_yaw.value = float(rot[0]); manual_pitch.value = float(rot[1]); manual_roll.value = float(rot[2])
	manual_x.value = float(trans[0]); manual_y.value = float(trans[1]); manual_z.value = float(trans[2])
...
func _refresh_manual_key_list() -> void:
	...
	for frame_value in frames:
		...
		var frame_data: Dictionary = manual_keys[frame]
		var xfm: Dictionary = frame_data.get("nodeXfm", {})
		var bones: Array = xfm.keys(); bones.sort()
		for bone_value in bones:
			var bone := str(bone_value)
			manual_key_list.add_item("F%04d  %-12s  %s" % [frame, bone, str(xfm[bone].get("rot", []))])
...
func _build_manual_animation() -> Dictionary:
	if manual_keys.is_empty():
		...
		return {}
	var frames: Array = manual_keys.keys(); frames.sort()
	var transforms := []
	for frame_value in frames:
		var frame := int(frame_value)
		var frame_data: Dictionary = manual_keys[frame]
		transforms.append({"frame":frame,"spline":str(frame_data.get("spline","LINEAR")),"nodeXfm":(frame_data.get("nodeXfm",{}) as Dictionary).duplicate(true)})
	var last_frame := int(frames.back())
	return {"category":"DEFAULT","frameCnt":maxi(last_frame,1),"frameRepeat":1,"animStart":0,"loopStart":0,"repeat":manual_loop_check.button_pressed,"transforms":transforms,"nodes":{},"manual_meta":{"timeline_fps":manual_fps_spin.value,"studio":"AlabasterBoneStudio"}}
...
func _save_manual() -> void:
	var data := _build_manual_animation()
	if data.is_empty(): return
	var name := _sanitize_name(manual_name_edit.text)
	if name.is_empty():
		_set_status("Choose a manual animation name.", true); return
	if not Library.save_custom_animation(name, data, {"type":"manual","studio":"AlabasterBoneStudio"}):
		_set_status("Could not save custom animation bank.", true); return
	rig.install_runtime_animation(name, data); rig.set_animation(name)
	_set_status("Saved manual animation '%s' to the animation bank." % name)


func _on_opacity_changed(value: float) -> void:
	if rig != null and rig.has_method("set_sprite_opacity"): rig.call("set_sprite_opacity", value)


func _on_bones_toggled(enabled: bool) -> void:
	if rig != null and rig.has_method("set_debug_enabled"): rig.call("set_debug_enabled", enabled)


func _on_facing_selected(index: int) -> void:
	if rig == null or index < 0: return
	var direction: Vector2 = facing_option.get_item_metadata(index)
	rig.set_facing_from_vector(direction)


func _manual_spline_value() -> String:
	return str(manual_spline.get_item_metadata(manual_spline.selected)) if manual_spline.selected >= 0 else "LINEAR"


func _add_heading(parent: Control, text: String) -> void:
	var label := Label.new(); label.text = text; label.add_theme_font_size_override("font_size", 16); parent.add_child(label)
...
func _add_row(parent: Control, label_text: String, control: Control) -> void:
	...
	var label := Label.new(); label.text = label_text; label.custom_minimum_size = Vector2(210,0); row.add_child(label)
	control.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(control); parent.add_child(row)


func _make_spin(value: float, minimum: float, maximum: float, step: float) -> SpinBox:
	var spin := SpinBox.new(); spin.min_value = minimum; spin.max_value = maximum; spin.step = step; spin.value = value; return spin


func _select_option_metadata(option: OptionButton, value: String) -> void:
	for i in range(option.item_count):
		if str(option.get_item_metadata(i)) == value: option.select(i); return
...
func _sanitize_name(value: String) -> String:
	...
	var clean := ""
	for ch in result:
		if ch.to_lower() in "abcdefghijklmnopqrstuvwxyz0123456789_": clean += ch
	return clean
