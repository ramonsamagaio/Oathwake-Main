...
func setup(new_velocity: Vector2, new_lifetime: float, new_size: float, new_colors: Array[Color], new_switch_interval: float, new_gravity := 0.0) -> void:
...
func _process(delta: float) -> void:
	...
	modulate.a = 1.0 - clampf(_age / lifetime, 0.0, 1.0)
	queue_redraw()
