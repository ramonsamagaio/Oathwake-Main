...
static func apply_glow(target: Node2D, config: Dictionary, default_glow_z := 24) -> Node2D:
	...
	_set_optional_property(glow, "perspective_angle_degrees", clampf(float(config.get("perspective_angle_degrees", 50.0)), 15.0, 90.0))
	...
	_set_optional_property(glow, "casts_night_shadows", bool(config.get("casts_night_shadows", true)))
