...
func _apply_fallback_visual() -> void:
	...
	if fallback_visual == null:
		fallback_visual = ColorRect.new()
		fallback_visual.name = "FallbackVisual"
		fallback_visual.mouse_filter = Control.MOUSE_FILTER_IGNORE
		add_child(fallback_visual)
		move_child(fallback_visual, 0)

	fallback_visual.size = Vector2(28, 24)
	fallback_visual.position = Vector2(-14, -18)
...
func _get_fallback_color() -> Color:
	match str(building_data.get("building_type", "")):
		"wall":
			return Color(0.50, 0.32, 0.18, 1.0)
		"bed":
			return Color(0.42, 0.34, 0.62, 1.0)
		"workstation":
			return Color(0.44, 0.28, 0.14, 1.0)
		_:
			return Color(0.55, 0.34, 0.12, 1.0)
