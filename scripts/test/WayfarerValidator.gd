...
func _run() -> void:
		...
		for direction in range(16):
			for sample in range(24):
				for rig in [original,character]:
					...
					rig.animation_time = duration * sample / 24.0
					rig._apply_pose()
				poses += 1
	...
	var file := FileAccess.open("res://docs/characters/wayfarer/rig-validation.json",FileAccess.WRITE)
	file.store_string(JSON.stringify(report,"\t"))
	print("WAYFARER_VALIDATION poses=%d regions=%d failures=%d missing=%d" % [poses,regions,failures.size(),character._compact_missing_region_warnings.size()])
	original.queue_free()
