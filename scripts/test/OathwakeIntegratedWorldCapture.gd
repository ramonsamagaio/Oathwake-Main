## Real procedural placements and the published tree catalogue. No save session.
extends SceneTree
...
func _run()->void:
	...
	await process_frame
	if not world._has_required_nodes():push_error("World capture has missing layers");quit(1);return
	...
	for index in range(targets.size()):
		...
		await RenderingServer.frame_post_draw
		canvas.get_texture().get_image().save_png(OUT+"world-%d.png"%index)
	print("INTEGRATED_WORLD_CAPTURE seed=74291 resources=",resources.get_child_count()," targets=",targets)
	quit()
