...
func _validate_player_light_stamina_hud_and_parry() -> void:
	...
	if sfx_manager == null or not sfx_manager.has_method("has_profile") or not bool(sfx_manager.call("has_profile", "player_parry")):
		...
		var attacker := BUTTERFLY_MONSTER_SCENE.instantiate() as CharacterBody2D
...
func _validate_particle_modes_scale_and_pet_runtime() -> void:
	...
	if particles == null:
		...
		if particles.color_ramp == null:
			...
			var colors := particles.color_ramp.colors
			if colors.is_empty() or colors[colors.size() - 1].a > 0.01:
	...
	await process_frame
	var pet_particles := pet.get_node_or_null("ContentParticles") as CPUParticles2D
...
func _validate_content_editor_field_contract() -> void:
	var editor_text := FileAccess.get_file_as_string("res://tools/content_editor/ContentEditorPetSaveDashSideSuite.gd")
	for token in [
	...
	var sfx_text := FileAccess.get_file_as_string("res://data/sfx_profiles.json")
