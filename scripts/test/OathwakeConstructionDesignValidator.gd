...
func _initialize() -> void:
	...
	if art.is_empty() or world_plan.is_empty(): _finish(failures); return
	...
	if failures.is_empty():
		print("OATHWAKE_BUILD_DESIGN_OK character=25.5x60.5 door=72 wall=88 floor_subtile=16 wall_sprites=9 stairs=32x64/64x32 road=48 trail=32 masks=16")
	_finish(failures)
...
func _validate_scale(art: Dictionary, failures: Array[String]) -> void:
	...
	var real: Dictionary = art.get("real_world_architecture", {})
	_expect(int(real.get("target_door_visual_height_px", 0)) == 72, "door must remain 72 px from real-world ratio", failures)
	_expect(int(real.get("target_wall_visual_height_px", 0)) == 88, "wall must remain 88 px from real-world ceiling ratio", failures)
	_expect(int(real.get("headwall_above_door_px", 0)) >= 14, "wall needs visible structure above the door", failures)
...
func _validate_assets(art: Dictionary, failures: Array[String]) -> void:
	...
	for wall_key in ["wall_horizontal", "wall_vertical", "wall_window_horizontal", "wall_window_vertical", "wall_doorway_horizontal", "wall_doorway_vertical"]:
		_expect(_pair(_entry(specs, wall_key).get("sprite_canvas_px", []), 32, 88), "%s canvas must remain 32x88" % wall_key, failures)
	...
	_expect(_pair(_entry(specs, "wall_joint_post").get("sprite_canvas_px", []), 16, 88), "joint post must remain 16x88", failures)
	_expect(_pair(_entry(specs, "door_leaf_horizontal").get("sprite_canvas_px", []), 32, 72), "horizontal door canvas must remain 32x72", failures)
	_expect(_pair(_entry(specs, "door_leaf_vertical").get("sprite_canvas_px", []), 32, 72), "vertical door canvas must remain 32x72", failures)
...
func _validate_wall_grammar(art: Dictionary, failures: Array[String]) -> void:
	...
	_expect(bool(readability.get("construction_only_cutaway", false)), "cutaway must be restricted to player construction", failures)
...
func _find_system(entries: Variant, system_name: String) -> Dictionary:
	...
	for value in entries as Array:
		if value is Dictionary and str((value as Dictionary).get("system", "")) == system_name: return value as Dictionary
	return {}
...
func _read_json(path: String, failures: Array[String]) -> Dictionary:
	if not FileAccess.file_exists(path): failures.append("missing file: %s" % path); return {}
	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(path))
	if not parsed is Dictionary: failures.append("invalid JSON dictionary: %s" % path); return {}
...
func _finish(failures: Array[String]) -> void:
	if not failures.is_empty():
		for failure in failures: push_error("OATHWAKE_BUILD_DESIGN_FAIL %s" % failure)
		quit(1); return
	quit(0)
