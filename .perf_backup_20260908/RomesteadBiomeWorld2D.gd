...
func _draw_shore(cell: Vector2i) -> void:
	if not _has_coastline_layers():
		return
	_draw_native_autotile(cell, TERRAIN_SAND, _shore_layers)
...
func _build_shore_texture() -> Texture2D:
	## beachtile.png tem 8 linhas (32 frames), mas AUTOTILE_MASK_SETUP indexa até
	## o frame 51. Sem preencher 32..51 a areia sairia com buracos justamente nas
	## máscaras "cheio". Os frames que faltam são todos máscara 31/24/18/20/17
	## (tile sólido) ou máscara 0 (vazio), então dá para montá-los a partir do
	## próprio tile sólido do sheet — sem depender de import nem de asset novo.
	...
	var columns := 4
	var frame_size := int(source.get_width() / columns)
	var total_frames := AUTOTILE_MASK_SETUP.size()
	var rows := int(ceil(float(total_frames) / float(columns)))
	var sheet := Image.create_empty(source.get_width(), rows * frame_size, false, Image.FORMAT_RGBA8)
	sheet.fill(Color(0.0, 0.0, 0.0, 0.0))
	sheet.blit_rect(source, Rect2i(Vector2i.ZERO, source.get_size()), Vector2i.ZERO)

	# O frame 6 é o tile totalmente preenchido — é o mesmo que _draw_native_autotile
	# usa quando a célula está inteira dentro do terreno.
	var solid_rect := Rect2i(Vector2i(6 % columns, 6 / columns) * frame_size, Vector2i(frame_size, frame_size))
	var source_frames := int(source.get_height() / frame_size) * columns
	for frame in range(source_frames, total_frames):
		if int(AUTOTILE_MASK_SETUP[frame]) == 0:
			continue
		sheet.blit_rect(source, solid_rect, Vector2i(frame % columns, frame / columns) * frame_size)
